  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data: response } = await axios.get(url,{params:{ size: searchCriteria.numberOfONGs, city:searchCriteria.city, county:searchCriteria.county, description:searchCriteria.description}});
        setData(response);
      } catch (error) {
        console.error(error)
      }
